  ...
  tar_target(
    ...
    {lapply(p1_rdsalt, 
            function(x) raster_to_catchment_polygons(polygon_sf = p1_catchments_sf_valid,
                                                     ...
                                                     new_cols_prefix = 'rd_slt',
                                                     na.rm = T))
    }
  ),
  
  # Aggregate to hru_segment for across each annual road salt df 
  # in list of p2_rdsalt_per_catchment
  tar_target(
  ...
  tar_target(
    ...
    lapply(list.files(path = p1_NADP_data_unzipped, full.names = T), 
           function(x) read.csv(x, sep = ',') %>%
             ...
             # using regex to extract the year between NADP_ and _CONUS
