summarize_site_list <- function(site_list_path,nwis_daily_data,nwis_inst_data,fileout){
  ...
  site_summary <- site_list %>%
    summarize(
           ...
           n_all_sites_WQP = as.numeric(tally(filter(.,grepl("WQP",.$data_src_combined,ignore.case=TRUE)))),
           # tally total number of NWIS-daily sites that contribute continuous data
           n_all_sites_nwis_daily = length(unique(nwis_daily_data$site_no)),
           # tally total number of NWIS-inst sites that contribute continuous data
           n_all_sites_nwis_inst = length(unique(nwis_inst_data$site_no)),
           # tally number of unique NWIS sites that contribute continuous data (daily or inst)
           n_unique_sites_NWIS = as.numeric(tally(filter(.,grepl("NWIS",.$data_src_combined,ignore.case=TRUE)))),
           ...
           n_unique_site_ids = length(unique(site_id)),
           ...
           n_obsdays = sum(count_days_total))
