...
SC_path <- tar_read(p2_wqp_SC_csv)
SC_data <- read_csv(SC_path,col_types = cols(ResultDetectionConditionText = col_character())) %>%
...
n_unique_sites_wqp <- SC_data %>% distinct(LongitudeMeasure,LatitudeMeasure)
n_unique_SC_locs <- length(n_unique_sites_wqp$LongitudeMeasure) - length(intersect(unique(SC_data$site_no),unique(SC_data_nwis$site_no)))
...
Combining the harmonized WQP dataset with daily data from NWIS (which includes data from NGWOS high-frequency stations), here is a map of site locations with specific conductance data (n = `r n_unique_SC_locs` sites):  

```{r,echo=FALSE}

# WQP sites: assume NAD83 for now, need to confirm (explicit datum missing in metadata?)
SC_sites_wqp <- sf::st_as_sf(SC_data,coords = c("LongitudeMeasure","LatitudeMeasure"),crs=4269) %>% 
  sf::st_transform(4326) %>%
  group_by(MonitoringLocationIdentifier) %>% 
  summarize(n=n()) 

# NWIS sites:
SC_nwis_data_summary <- SC_data_nwis %>% group_by(site_no) %>% summarize(n_days=n())

SC_nwis_site_summary <- filter(SC_sites_nwis,data_type_cd=="dv",stat_cd=="00003") %>%
  select(site_no,station_nm,site_tp_cd,dec_lat_va,dec_long_va,dec_coord_datum_cd,parm_cd,stat_cd) %>%
  group_by(site_no) %>%
  slice(1) %>%
  left_join(.,SC_nwis_data_summary,by="site_no") %>%
...
leaflet() %>%
  ...
  addCircleMarkers(data=SC_sites_wqp,radius = ~sqrt(n/50),color ="orange",
    stroke = FALSE, fillOpacity = 0.5,popup=paste("Site:", SC_sites_wqp$MonitoringLocationIdentifier,"<br>",
                                                  "n_records:", SC_sites_wqp$n)) %>%
  addCircleMarkers(data=SC_nwis_site_summary,radius = ~sqrt(n_days/50),color ="green",
    stroke = FALSE, fillOpacity = 0.5,popup=paste("Site:", SC_nwis_site_summary$site_no,"<br>",
                                                  "n_daily_records:",SC_nwis_site_summary$n_days)) %>%
