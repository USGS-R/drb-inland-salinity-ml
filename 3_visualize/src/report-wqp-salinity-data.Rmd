...
SpC_data <- read_csv(SpC_path,col_types = cols(ResultDetectionConditionText = col_character())) %>%
...
# Load NWIS daily SpC sites and data
SpC_sites_nwis <- tar_read(p1_nwis_SpC_sites)
SpC_data_nwis_path <- tar_read(p2_nwis_SpC_daily_csv)
SpC_data_nwis <- read_csv(SpC_data_nwis_path,show_col_types = FALSE)
...
# How many truly unique sites are there?
n_unique_sites_wqp <- SpC_data %>% distinct(LongitudeMeasure,LatitudeMeasure)
n_unique_SpC_locs <- length(n_unique_sites_wqp$LongitudeMeasure) - length(intersect(unique(SpC_data$site_no),unique(SpC_data_nwis$site_no)))



```
  
### Harmonized WQP dataset
...
Combining the harmonized WQP dataset with daily data from NWIS (which includes data from NGWOS high-frequency stations), here is a map of site locations with specific conductance data (n = `r n_unique_SpC_locs` sites):  

```{r,echo=FALSE}

# WQP sites: assume NAD83 for now, need to confirm (explicit datum missing in metadata?)
SpC_sites_wqp <- sf::st_as_sf(SpC_data,coords = c("LongitudeMeasure","LatitudeMeasure"),crs=4269) %>% 
  sf::st_transform(4326) %>%
  group_by(MonitoringLocationIdentifier) %>% 
  summarize(n=n()) 

# NWIS sites:
SpC_nwis_data_summary <- SpC_data_nwis %>% group_by(site_no) %>% summarize(n_days=n())

SpC_nwis_site_summary <- filter(SpC_sites_nwis,data_type_cd=="dv",stat_cd=="00003") %>%
  select(site_no,station_nm,site_tp_cd,dec_lat_va,dec_long_va,dec_coord_datum_cd,parm_cd,stat_cd) %>%
  group_by(site_no) %>%
  slice(1) %>%
  left_join(.,SpC_nwis_data_summary,by="site_no") %>%
...
leaflet() %>%
  ...
  addCircleMarkers(data=SpC_sites_wqp,radius = ~sqrt(n/50),color ="orange",
    stroke = FALSE, fillOpacity = 0.5,popup=paste("Site:", SpC_sites_wqp$MonitoringLocationIdentifier,"<br>",
                                                  "n_records:", SpC_sites_wqp$n)) %>%
  addCircleMarkers(data=SpC_nwis_site_summary,radius = ~sqrt(n_days/50),color ="green",
    stroke = FALSE, fillOpacity = 0.5,popup=paste("Site:", SpC_nwis_site_summary$site_no,"<br>",
                                                  "n_daily_records:",SpC_nwis_site_summary$n_days)) %>%
  addLegend("bottomright", colors = c("orange","green"), labels = c("Harmonized WQP dataset","NWIS daily data")) 
