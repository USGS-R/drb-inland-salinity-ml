...
# Load full site list:
site_list <- read_csv(tar_read(p2_site_list_csv),show_col_types = FALSE)
unique_all_lat_lon <- site_list %>% distinct(lat,lon) %>% summarize(n_sites=n())
...
Combining the harmonized WQP dataset with daily data from NWIS (which includes data from NGWOS high-frequency stations), here is a map of site locations with specific conductance data (n = `r unique_all_lat_lon$n_sites` unique lat/lon locations):  

```{r,echo=FALSE}

# Handle different coordinate reference systems across sites:
site_list_split <- split(site_list,f=site_list$datum)
site_list_sp_ls <- lapply(site_list_split,function(x){
  
  # assume unknown crs (datum = "UNKWN") are equal to WGS84:
  datum_assign <- case_when(x$datum[1] == "NAD83" ~ 4269,
                            x$datum[1] == "WGS84" ~ 4326,
                            x$datum[1] == "NAD27" ~ 4267,
                            x$datum[1] == "UNKWN" ~ 4326)
  
  # convert data frame to spatial object:
  site_list_sp <- sf::st_as_sf(x,coords=c("lon","lat"),crs=datum_assign) %>%
...
site_list_sp <- do.call(rbind,site_list_sp_ls)

# Define color palette for circle markers:
pal <- colorFactor(palette = c("orange","blue","green"),domain=site_list_sp$group)
