aggregate_proportions_hrus <- function(df, group_by_segment_colname, proportion_col_prefix, hru_area_colname, new_area_colname){
  ...
  #'@description aggregation ufnction to get land cover class proportions for PRMS_catchment_area 
  ...
  df <- df %>% 
    # Create temp cols of area of lc class per hru - NOTE: simply mutate current cols, so col names are not rep of value as its not a prop. This is only temp 
