reclassify_land_cover <- function(land_cover_df,
  ...
  # Load reclassification csv only grabing values, not description cols
  ...
  pivoted_merged_df <- as.data.frame(land_cover_df) %>% 
    # pivot longer sinc that old class is the original land cover class label (1,3,4,...) and land cover class values fall in a prop class in catchment call. (this can be renamed when generalizing)
  ...
  # print(pivoted_merged_df)gti
  new_classes_df <- pivoted_merged_df %>% 
